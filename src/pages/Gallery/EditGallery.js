...
const EditGallery = () => {
  ...
  const [vtubersource, setVtubersource] = useState("");
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:4000/image/" + id).then((response) => {
      response.json().then((imageInfo) => {
        ...
        setVtubername(imageInfo.vtubername);
        setVtubersource(imageInfo.vtubersource);
        setArt(imageInfo.art);
        setLink(imageInfo.link);
      });
    });
  }, []);
  ...
  async function updateImage(ev) {
    ev.preventDefault();
    const data = new FormData();
    data.set("title", title);
    data.set("vtubername", vtubername);
    data.set("vtubersource", vtubersource);
    data.set("art", art);
    data.set("link", link);
    data.set("id", id);
    if (files?.[0]) {
    ...
    <>
      <div className='container mx-auto flex flex-wrap py-6'>
        <section className='w-full flex flex-col items-center px-3'>
                      ...
                      <div className='mt-2'>
                        <div className='flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md'>
                          <input
                            ...
                            id='vtubername'
                            value={vtubername}
                            onChange={(ev) => setVtubername(ev.target.value)}
                            className='block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6'
                            placeholder='Input vtuber name'
                          />
                        </div>
                      </div>
                    </div>
                      ...
                      <div className='mt-2'>
                        <div className='flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md'>
                          <input
                            ...
                            id='vtubersource'
                            value={vtubersource}
                            onChange={(ev) => setVtubersource(ev.target.value)}
                            className='block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6'
                            placeholder='Input vtuber source'
                          />
                        </div>
                      </div>
                    </div>
                      ...
                      <div className='mt-2'>
                        <div className='flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md'>
                          <input
                            ...
                            id='art'
                            value={art}
                            onChange={(ev) => setArt(ev.target.value)}
                            className='block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6'
                            placeholder='Create new Art'
                          />
                        </div>
                      </div>
                    </div>
          ...
          {!username && <h1>Kamu tidak login atau terautentikasi</h1>}
        </section>
      </div>
    </>
  );
